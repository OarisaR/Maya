...
function calcTrimester(week: number, lang: "en" | "bn"): string {
  if (week <= 12) return tr("tracker_ov_trimester1", lang);
  if (week <= 27) return tr("tracker_ov_trimester2", lang);
  return tr("tracker_ov_trimester3", lang);
}
...
function bmiStatus(bmi: number, lang: "en" | "bn"): { label: string; color: string } {
  if (bmi < 18.5) return { label: tr("tracker_bmi_underweight", lang), color: "text-yellow-500" };
  if (bmi < 25)   return { label: tr("tracker_bmi_normal", lang),      color: "text-green-500" };
  if (bmi < 30)   return { label: tr("tracker_bmi_overweight", lang),  color: "text-orange-500" };
  return             { label: tr("tracker_bmi_obese", lang),       color: "text-red-500" };
}
...
function OverviewPanel({ week, profile, weightHistory, labValues, lang }: {
  week: number;
  ...
  const info = getWeeklyInfo(week);
  const trimester = calcTrimester(week, lang);
  ...
  return (
    <div className="space-y-6">
      <h3 className="text-lg font-semibold">{tr("tracker_ov_title", lang)}</h3>

      <div className="rounded-2xl border border-border bg-card p-5 space-y-4">
        <div className="flex items-center justify-between">
          <div>
            <div className="text-sm text-muted-foreground">
              {tr("tracker_week_of_40_full", lang).replace("{week}", String(week))}
            </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="text-sm">
            {tr("tracker_ov_baby_size", lang)}{" "}
            <span className="font-semibold text-primary">{info.size}</span>
          </div>
        </div>
        ...
        <div className="flex justify-between text-xs text-muted-foreground">
          <span>
            {daysLeft
              ? tr("tracker_ov_days_left", lang).replace("{days}", String(daysLeft))
              : tr("tracker_ov_due_not_set", lang)}
          </span>
          <span>{tr("tracker_ov_percent", lang).replace("{percent}", String(Math.round(progress)))}</span>
        </div>
      </div>
      ...
      {profile && (
        <div className="grid grid-cols-3 gap-3">
          <div className="rounded-xl bg-muted/40 p-3 text-center">
            ...
            <div className="text-xs text-muted-foreground">{tr("tracker_ov_kg", lang)}</div>
          </div>
          <div className="rounded-xl bg-muted/40 p-3 text-center">
            <div className="text-xs text-muted-foreground mb-0.5">{tr("tracker_ov_bmi", lang)}</div>
            <div className={`text-lg font-semibold ${bmi ? bmiStatus(bmi, lang).color : ""}`}>
              {bmi || "—"}
            </div>
            {bmi && <div className="text-[10px] text-muted-foreground">{bmiStatus(bmi, lang).label}</div>}
          </div>
          <div className="rounded-xl bg-muted/40 p-3 text-center">
            <FlaskConical className="w-4 h-4 mx-auto mb-1 text-primary" />
            <div className="text-lg font-semibold">{latestHb ? `${latestHb.value}` : "—"}</div>
            <div className="text-xs text-muted-foreground">{tr("tracker_ov_hb", lang)}</div>
          </div>
        </div>
      )}
    </div>
...
function HealthPanel({ profile, weightHistory, labValues, week, lang }: {
  profile: HealthProfile | null;
  weightHistory: WeightEntry[];
  labValues: LabValue[];
  week: number;
  ...
  const TYPE_LABELS: Record<string, string> = {
    hb:            tr("tracker_lab_hb", lang),
    glucose:       tr("tracker_lab_glucose", lang),
    tsh:           tr("tracker_lab_tsh", lang),
    ferritin:      tr("tracker_lab_ferritin", lang),
    urine_protein: tr("tracker_lab_urine_protein", lang),
  };

  const conditionLabel: Record<string, string> = {
    diabetes:    tr("tracker_cond_diabetes", lang),
    hypertension:tr("tracker_cond_hypertension", lang),
    thyroid:     tr("tracker_cond_thyroid", lang),
    anaemia:     tr("tracker_cond_anaemia", lang),
  };
  ...
  return (
    <div className="space-y-6">
      <h3 className="text-lg font-semibold">{tr("tracker_hp_title", lang)}</h3>
      ...
      {profile && (
        <div className="rounded-2xl border border-border bg-card p-5 space-y-4">
          <div className="flex items-center gap-2 text-sm font-medium">
            ...
            {tr("tracker_hp_snapshot", lang)}
          </div>

          <div className="grid grid-cols-2 gap-3">
            <div className="rounded-xl bg-muted/40 p-3">
              <div className="text-xs text-muted-foreground">{tr("tracker_hp_bmi", lang)}</div>
              <div className={`text-lg font-semibold ${bmi ? bmiStatus(bmi, lang).color : ""}`}>
                {bmi || "—"}
              </div>
              {bmi && <div className="text-xs text-muted-foreground">{bmiStatus(bmi, lang).label}</div>}
            </div>
            <div className="rounded-xl bg-muted/40 p-3">
              <div className="text-xs text-muted-foreground">{tr("tracker_hp_weight", lang)}</div>
              <div className="text-lg font-semibold">{profile.weightKg || "—"} {tr("tracker_ov_kg", lang)}</div>
            </div>
            <div className="rounded-xl bg-muted/40 p-3">
              <div className="text-xs text-muted-foreground">{tr("tracker_hp_blood_type", lang)}</div>
              <div className="text-lg font-semibold">{profile.bloodType || "—"}</div>
            </div>
            <div className="rounded-xl bg-muted/40 p-3">
              <div className="text-xs text-muted-foreground">{tr("tracker_hp_due_date", lang)}</div>
      ...
      {weightHistory.length > 1 ? (
        <div className="rounded-2xl border border-border bg-card p-5 space-y-4">
          <div className="flex items-center gap-2 text-sm font-medium">
            ...
            {tr("tracker_hp_weight_gain", lang)}
          </div>
          <div className="h-48 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={weightHistoryWithWeek} margin={{ top: 5, right: 5, left: -20, bottom: 5 }}>
                ...
                <Tooltip
                  formatter={(value: number) => [`${value} ${tr("tracker_ov_kg", lang)}`, tr("tracker_hp_weight_tooltip", lang)]}
                  labelFormatter={(w: number) => tr("tracker_hp_week_label", lang).replace("{week}", String(w))}
                />
                <Line
                  type="monotone"
                  dataKey="weight"
          ...
          <div className="grid grid-cols-2 gap-3 text-sm">
            <div>
              <div className="text-xs text-muted-foreground">{tr("tracker_hp_starting_weight", lang)}</div>
              <div className="font-semibold">{weightHistory[0]?.weight} {tr("tracker_ov_kg", lang)}</div>
              <div className="text-xs text-muted-foreground">
                {tr("tracker_hp_week_label", lang).replace("{week}", String(weightHistory[0]?.week))}
              </div>
            </div>
            <div>
              <div className="text-xs text-muted-foreground">{tr("tracker_hp_current", lang)}</div>
              <div className="font-semibold">{profile?.weightKg} {tr("tracker_ov_kg", lang)}</div>
              <div className="text-xs text-muted-foreground">
                {tr("tracker_hp_week_label", lang).replace("{week}", String(week))}
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-border bg-card p-5 space-y-3">
          ...
          <p className="text-sm text-muted-foreground">
            {weightHistory.length === 1
              ? tr("tracker_hp_one_weight", lang)
                  .replace("{weight}", String(weightHistory[0].weight))
                  .replace("{week}", String(weightHistory[0].week))
              : tr("tracker_hp_no_weight", lang)}
          </p>
        </div>
      )}
      ...
      <div className="rounded-2xl border border-border bg-card p-5 space-y-4">
        <div className="flex items-center gap-2 text-sm font-medium">
          ...
          {tr("tracker_hp_lab_results", lang)}
        </div>
        {labValues.length > 0 ? (
            ...
            {latestByType.map((val) => (
              <div key={val.id} className="flex items-center justify-between py-2 border-b border-border/50 last:border-0">
                <div>
                  ...
                  {val.reportRange && (
                    <div className="text-xs text-muted-foreground">
                      {tr("tracker_hp_range", lang).replace("{range}", val.reportRange)}
                    </div>
                  )}
                </div>
          ...
          <p className="text-sm text-muted-foreground">{tr("tracker_hp_no_lab", lang)}</p>
        )}
      </div>
    </div>
  );
...
function AppointmentsPanel({ appointments, suggestions, week, uid, onUpdate, lang }: {
  ...
  return (
    <div className="space-y-6">
      <h3 className="text-lg font-semibold">{tr("tracker_ap_title", lang)}</h3>

      {/* Add Button */}
      <Button onClick={() => setShowAdd(true)} className="w-full">
        <Plus className="w-4 h-4 mr-2" /> {tr("tracker_ap_add_btn", lang)}
      </Button>
      ...
      {showAdd && (
        <div className="rounded-2xl border border-border bg-card p-5 space-y-4">
          <div className="flex items-center justify-between">
            <div className="text-sm font-medium">{tr("tracker_ap_add_form_title", lang)}</div>
          ...
          <div className="space-y-3">
            <div className="space-y-2">
              <Label className="text-xs">{tr("tracker_ap_type", lang)}</Label>
              <select
                className="w-full rounded-md border border-input bg-background px-3 py-2 text-sm"
                value={selectedType}
                onChange={(e) => setSelectedType(e.target.value)}
              >
                <option value="routine_checkup">{tr("tracker_ap_opt_routine", lang)}</option>
                <option value="dating_scan">{tr("tracker_ap_opt_dating", lang)}</option>
                <option value="anomaly_scan">{tr("tracker_ap_opt_anomaly", lang)}</option>
                <option value="gtt">{tr("tracker_ap_opt_gtt", lang)}</option>
                <option value="gbs">{tr("tracker_ap_opt_gbs", lang)}</option>
                <option value="custom">{tr("tracker_ap_opt_custom", lang)}</option>
              </select>
            </div>
            {selectedType === "custom" && (
              <div className="space-y-2">
                <Label className="text-xs">{tr("tracker_ap_title_label", lang)}</Label>
                <Input
                  value={customTitle}
                  onChange={(e) => setCustomTitle(e.target.value)}
                  placeholder={tr("tracker_ap_title_ph", lang)}
                />
              </div>
            )}
            <div className="space-y-2">
              <Label className="text-xs">{tr("tracker_ap_date", lang)}</Label>
              <Input type="date" value={date} onChange={(e) => setDate(e.target.value)} />
            </div>
            <div className="space-y-2">
              <Label className="text-xs">{tr("tracker_ap_time", lang)}</Label>
              <Input type="time" value={time} onChange={(e) => setTime(e.target.value)} />
            </div>
            <div className="space-y-2">
              <Label className="text-xs">{tr("tracker_ap_location", lang)}</Label>
              <Input
                value={location}
                onChange={(e) => setLocation(e.target.value)}
                placeholder={tr("tracker_ap_location_ph", lang)}
              />
            </div>
            <Button onClick={handleAdd} className="w-full" disabled={!date}>
              {tr("tracker_ap_save", lang)}
            </Button>
          </div>
        </div>
      )}
      ...
      {visibleSuggestions.map((sug) => (
        <div key={sug.id} className="rounded-xl bg-primary/5 border border-primary/20 p-4 space-y-3">
          ...
          <div className="text-xs text-muted-foreground">
            {tr("tracker_ap_recommended_at", lang).replace("{week}", String(sug.weekSuggested))}
          </div>
          <div className="flex gap-2">
            <Button size="sm" onClick={() => {
              ...
              <Plus className="w-3 h-3 mr-1" /> {tr("tracker_ap_add_small", lang)}
            </Button>
            <Button variant="ghost" size="sm" onClick={() => handleDismiss(sug)}>
              <X className="w-3 h-3 mr-1" /> {tr("tracker_ap_dismiss", lang)}
            </Button>
          </div>
        </div>
      ))}
      ...
      {trulyUpcoming.length > 0 && (
        <div className="space-y-3">
          <div className="text-sm font-medium">{tr("tracker_ap_upcoming", lang)}</div>
          {trulyUpcoming.map((a) => (
            <div key={a.id} className="rounded-xl bg-muted/40 p-4 space-y-2">
              <div className="flex items-center justify-between">
                ...
                <Button variant="ghost" size="sm" className="h-7" onClick={() => handleComplete(a.id)}>
                  <Check className="w-3 h-3 mr-1" /> {tr("tracker_ap_done", lang)}
                </Button>
              </div>
              ...
              <div className="text-xs font-medium">
                {daysUntil(a.date) > 0
                  ? tr("tracker_ap_in_days", lang).replace("{days}", String(daysUntil(a.date)))
                  : daysUntil(a.date) === 0
                    ? tr("tracker_ap_today", lang)
                    : tr("tracker_ap_overdue", lang)}
              </div>
            </div>
          ))}
        </div>
      ...
      {missed.length > 0 && (
        <div className="space-y-3">
          <div className="text-sm font-medium text-red-400">{tr("tracker_ap_missed", lang)}</div>
          {missed.map((a) => (
            <div key={a.id} className="rounded-xl bg-red-500/5 border border-red-500/10 p-4 space-y-2 opacity-70">
              <div className="flex items-center justify-between">
                ...
                >
                  <Check className="w-3 h-3 mr-1" /> {tr("tracker_ap_mark_done", lang)}
                </Button>
              </div>
              ...
              <div className="text-xs font-medium text-red-400/60">
                {tr("tracker_ap_days_overdue", lang).replace("{days}", String(Math.abs(daysUntil(a.date))))}
              </div>
            </div>
          ))}
        </div>
      ...
      {completed.length > 0 && (
        <div className="space-y-3">
          <div className="text-sm font-medium text-muted-foreground">{tr("tracker_ap_completed", lang)}</div>
      ...
      {!suggestions.length && !trulyUpcoming.length && !missed.length && !completed.length && (
        <p className="text-sm text-muted-foreground text-center py-8">
          {tr("tracker_ap_no_appointments", lang)}
        </p>
      )}
    </div>
  );
...
export function PregnancyTracker({ week, setWeek, lang, user, defaultTab, open, onOpenChange, trigger }: TrackerProps) {
  ...
  // Sidebar items built with translated labels
  const sidebarItems: { id: PanelType; icon: React.ElementType; label: string }[] = [
    { id: "overview",      icon: Activity, label: tr("tracker_nav_overview", lang) },
    { id: "health",        icon: Heart,    label: tr("tracker_nav_health", lang) },
    { id: "appointments",  icon: Calendar, label: tr("tracker_nav_appointments", lang) },
  ];
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      <DialogContent className="w-[680px] max-w-[90vw] h-[560px] max-h-[85vh] p-0 gap-0 overflow-hidden flex flex-col">
        ...
        <div className="flex flex-1 overflow-hidden">
          {/* Left Sidebar */}
          <div className="w-[190px] border-r border-border gap-2 bg-muted/30 flex flex-col pt-3 shrink-0">
